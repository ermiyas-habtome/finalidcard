...
TIF_TEMPLATE = r"C:\ID/final3/file/template.tif"
FONT_PATH = r"C:\ID\final3\file\noto\static\NotoSansEthiopic_Condensed-Bold.ttf"
BORDER_TEMPLATE_PATH = r"C:\ID/final3/file/border PNG.png"
FONT_SIZE = 72
TEXT_COLOR = (0, 0, 0)
PHOTO_SIZE = (885, 1171)
QR_SIZE = (1501, 1357)
...
# -------------------- ID Card Generation --------------------
# At the top, make sure to import datetime
from datetime import datetime
...
def generate_id_card(data, txt_data):
    ...
    if name and str(name).lower() != "nan":
        ...
        if issue_date_ec and str(issue_date_ec).lower() not in {"nan", ""}:
            try:
                rotated_text_img = Image.new("RGBA", (500, 80), (255, 255, 255, 0))
        ...
        if row.get("PhotoPath") and os.path.exists(row["PhotoPath"]):
            try:
                ...
                clone_size = (230, 280)
                clone_position = (2350, 1397)
                clone_photo = original_photo.resize(clone_size, resample=Image.LANCZOS)
                from PIL import ImageFilter
                clone_photo = clone_photo.filter(ImageFilter.GaussianBlur(radius=2))
    ...
    if not name_clean or name_clean.lower() == "n/a":
        name_value = f"id_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    else:
        name_value = name_clean.replace(" ", "_").replace("/", "_").replace("\\", "_")

    png_out = os.path.join(OUTPUT_DIR, f"{name_value}.png")

    # ✅ Save optimized PNG
    flipped_png = base_img.transpose(Image.FLIP_LEFT_RIGHT)
    flipped_png.save(png_out, format="PNG", optimize=True)
    ...
    # Always return two values even if skipped
    return None, None
...
def extract_from_pdf(pdf_path, output_dir, number, original_filename):
    ...
    return [
        ...
        f"{today_ec.year:04d}/{today_ec.month:02d}/{today_ec.day:02d}",
        f"{expiry_ec.year:04d}/{expiry_ec.month:02d}/{expiry_ec.day:02d}"
    ], text_path
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Process PDF and get output image paths
        flipped_png, border_png = process_single_pdf(uploaded_path, output_dir, 2, doc.file_name)

        # Function to send and delete a file
        async def send_and_delete_file(file_path):
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    os.remove(file_path)
                    print(f"🗑️ Deleted after sending: {file_path}")
                except Exception as e:
                    print(f"❌ Failed to send or delete {file_path}: {e}")
                    await update.message.reply_text(f"❌ Could not send or delete {os.path.basename(file_path)}")
            else:
                await update.message.reply_text(f"❌ File not found: {os.path.basename(file_path) if file_path else 'Unknown'}")

        # Send and delete both images
        await send_and_delete_file(flipped_png)
        await send_and_delete_file(border_png)

        # ✅ Delete all PNG files from 'psd' folder
        psd_folder = "psd"  # Adjust this path if needed
        if os.path.exists(psd_folder):
            try:
                for file in os.listdir(psd_folder):
                    if file.lower().endswith(".png"):
                        file_path = os.path.join(psd_folder, file)
                        os.remove(file_path)
                        print(f"🗑️ Deleted PNG from psd: {file_path}")
            except Exception as e:
                print(f"❌ Error deleting PNGs from psd: {e}")
    ...
    folders_to_delete = [user_folder, output_dir]
    ...
    await update.message.reply_text("✅ ID processing complete. Files sent.")
...
if __name__ == "__main__":
    main()
#THANKS GOD FOR ALLOFF THIS, NEXT
